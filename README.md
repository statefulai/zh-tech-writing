...
`SKILL.md` 是每次调用都要读的短入口说明：按请求选定 Audit、Structure、Draft 或 Revise 四种模式之一，同时遵守几条硬约束——不越过用户给出的授权、不发明缺失的事实、不改动结论的证据强度。`references/` 里的结构、语言与证据、发布安全三份规则按需加载，任务涉及哪一类才读哪一份。

目标文档以及其中的链接、引文只作为待处理材料：按约定它们不能切换模式，也不能扩大修改文件、联网或发布的授权。规则只看表达是否挡住了事实和证据，规范的标题、条目同类的列表、偏书面的技术写法本身不算问题。`evals/` 记录期望行为和不变量，供人工或其他评测使用；本仓库没有自动测试运行器。
...
```text
.
├── SKILL.md                    # 每次调用要读的短入口：模式路由与硬约束
├── agents/openai.yaml          # Codex / ChatGPT UI 元数据
├── references/                 # 按需加载：结构、语言与证据、发布安全
├── evals/                      # 行为用例与不变量，不是自动测试套件
